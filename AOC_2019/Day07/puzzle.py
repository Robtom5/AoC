...
class IntComputer():
    ...
    def addToInput(self, input):
        self.inputQueue.put(input)
        return self

    def setOutputQueue(self, target):
        self.outputQueue = target
        return self
    ...
    def _OP_OUT(self, trgt):
        if (self.outputQueue != None):
            self.outputQueue.put(self.commands[trgt])
        else:
...
amp5 = IntComputer().loadCommands('source.txt').setOutputQueue(out)
amp4 = IntComputer().loadCommands('source.txt').setOutputQueue(amp5.inputQueue)
amp3 = IntComputer().loadCommands('source.txt').setOutputQueue(amp4.inputQueue)
amp2 = IntComputer().loadCommands('source.txt').setOutputQueue(amp3.inputQueue)
amp1 = IntComputer().loadCommands('source.txt').setOutputQueue(amp2.inputQueue)
...
for perm in list(perms):
    amp1.reloadCommands().addToInput(perm[0]).addToInput(0)
    amp2.reloadCommands().addToInput(perm[1])
    amp3.reloadCommands().addToInput(perm[2])
    amp4.reloadCommands().addToInput(perm[3])
    amp5.reloadCommands().addToInput(perm[4])
...
## PART 2
# Need to change the queues to block until done, and spin up a thread per thruster, then go until all tasks complete
import threading
...
best = None
bestThrust = 0
amp5.setOutputQueue(amp1.inputQueue)
...
for perm in list(perms):
    ...
    amp4.reloadCommands().addToInput(perm[3])
    amp5.reloadCommands().addToInput(perm[4])
    threads.append(threading.Thread(target=amp1.execute))
    threads.append(threading.Thread(target=amp2.execute))
    threads.append(threading.Thread(target=amp3.execute))
    threads.append(threading.Thread(target=amp4.execute))
    threads.append(threading.Thread(target=amp5.execute))
    ...
    res = amp1.inputQueue.get_nowait()
    if (res > bestThrust):
        best = perm
        bestThrust = res
